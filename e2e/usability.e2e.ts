...
test('keeps common row controls visible and hides expert settings until requested', async ({ page }) => {
  ...
  await expect(page.getByLabel('Количество элементов')).toBeVisible()
  await expect(page.getByLabel('Ориентация')).toBeVisible()
  await expect(page.getByRole('button', { name: 'Дополнительно' })).toHaveAttribute('aria-expanded', 'false')
  await expect(page.getByRole('button', { name: 'Раппорт', exact: true })).toHaveCount(0)
  ...
  await expect(page.getByRole('button', { name: 'Раппорт', exact: true })).toBeVisible()
  await expect(page.getByRole('button', { name: 'Замкнутый', exact: true })).toBeVisible()
})
