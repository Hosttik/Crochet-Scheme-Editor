...
test('panel 2 follows the reference command hierarchy and keeps its controls live', async ({ page }) => {
  ...
  const libraryFavorites = page.getByRole('region', { name: 'Библиотека элементов' }).getByTestId('favorites-section')
  await expect(libraryFavorites).toBeVisible()
  await expect(libraryFavorites.locator('.favorite-item-button')).toHaveCount(3)
})
